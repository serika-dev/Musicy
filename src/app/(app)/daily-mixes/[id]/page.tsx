...
export default function DailyMixPage() {
  ...
  if (isLoading) {
    return (
      <div className="space-y-8">
        <div className="h-[40vh] lg:h-[45vh] w-full animate-pulse rounded-3xl bg-muted" />
  ...
  if (error || !dailyMix) {
    const expired = error?.message === "Daily mix has expired";
    return (
      <EmptyState
        icon={expired ? <Clock /> : <Music />}
        title={expired ? "Mix expired" : "Daily mix not found"}
        description={
          expired
  ...
  const totalDuration =
    dailyMix.tracks?.reduce((acc, track) => acc + (track.duration || 0), 0) ||
    0;
          ...
          <div className="space-y-4 lg:pb-4 flex-1 w-full">
            ...
            <div className="flex items-center gap-4 text-xs font-bold text-foreground/70">
              <span className="flex items-center gap-1.5">
                ...
                {dailyMix.user?.displayName ||
                  dailyMix.user?.username ||
                  "Musicy"}
              </span>
              <span>•</span>
      ...
      <div className="sticky top-16 z-30 -mx-4 lg:-mx-8 px-4 lg:px-8 bg-background/80 backdrop-blur-xl border-b border-white/5 lg:static lg:mx-0 lg:px-0 lg:bg-transparent lg:border-none">
        <div className="py-4 flex items-center justify-between">
          <div className="flex items-center gap-4 lg:gap-6">
            ...
            >
              {currentTrack &&
              dailyMix.tracks?.some((t) => t.id === currentTrack.id) &&
              isPlaying ? (
              ...
              <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
                ...
                <DialogContent className="max-w-md rounded-[2rem] border-white/10 shadow-2xl p-6">
                  ...
                  <div className="space-y-4">
                    <div>
                      <label
                        htmlFor="mix-playlist-name"
                        className="text-xs font-black uppercase text-foreground/40 tracking-widest block mb-2"
                      >
                        Playlist Name
                      </label>
                      <Input
                        id="mix-playlist-name"
                        value={playlistName}
                        onChange={(e) => setPlaylistName(e.target.value)}
                        className="h-12 rounded-xl font-medium bg-white/5 border-white/10 focus-visible:ring-primary/50"
                      />
                    </div>
                    <div>
                      <label
                        htmlFor="mix-playlist-desc"
                        className="text-xs font-black uppercase text-foreground/40 tracking-widest block mb-2"
                      >
                        Description
                      </label>
                      <Textarea
                        id="mix-playlist-desc"
                        value={playlistDescription}
                        onChange={(e) => setPlaylistDescription(e.target.value)}
                        className="rounded-xl font-medium bg-white/5 border-white/10 focus-visible:ring-primary/50 resize-none"
                        rows={3}
                      />
                    </div>
                    <Button
                      size="lg"
                      className="w-full rounded-xl font-bold font-lg h-12"
                      onClick={handleSavePlaylist}
                      disabled={
                        savePlaylistMutation.isPending || !playlistName.trim()
                      }
                    >
            ...
            <Button
              variant="outline"
              size="icon"
              className="w-12 h-12 rounded-full lg:hidden border-white/10 flex-shrink-0"
            >
          ...
          <Button
            variant="ghost"
            size="icon"
            className="w-12 h-12 rounded-full hidden lg:flex border-white/10 hover:bg-white/5"
          >
            <Share className="h-5 w-5" />
          </Button>
        </div>
      ...
      <div className="space-y-6">
        ...
        <div className="space-y-1">
          {dailyMix.tracks && dailyMix.tracks.length > 0 ? (
              ...
              >
                ...
                <div className="flex-1 overflow-hidden">
                  <TrackListItem
                    ...
                    onPlay={() =>
                      playTrack(track, dailyMix.tracks, {
                        type: "daily-mix",
                        id: dailyMix.id,
                        name: dailyMix.name,
                      })
            ...
            <EmptyState icon={<Music />} title="This mix is empty" />
          )}
        </div>
      </div>
    </div>
