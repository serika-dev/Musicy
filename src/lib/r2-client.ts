...
export async function uploadFileToB2(
  ...
  throw new Error(`Failed to upload to B2 after ${maxRetries} attempts: ${lastError?.message || 'Unknown error'}`)
}
...
export async function uploadLargeFileToB2(
  ...
  try {
    const createResult = await b2Client.send(new CreateMultipartUploadCommand({
      Bucket: B2_BUCKET,
      Key: key,
      ContentType: contentType,
    }))
    ...
    for (let index = 0; index < chunks.length; index++) {
      ...
      const result = await b2Client.send(new UploadPartCommand({
        Bucket: B2_BUCKET,
        Key: key,
    ...
    if (UploadId) {
      try {
        const { AbortMultipartUploadCommand: AbortCmd } = await import('@aws-sdk/client-s3')
        await b2Client.send(new AbortCmd({ Bucket: B2_BUCKET, Key: key, UploadId }))
